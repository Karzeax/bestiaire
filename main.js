...
// uniquement quand le mot ouvre une ligne (précédé d'un <br> ou en tête de chaîne).
// Les mentions en plein milieu d'une phrase (ex: "comme Réussite") ne sont PAS décorées.
// "Semi-réussite" matché avant "Réussite" pour éviter les collisions de remplacement.
function decorateOutcomes(html) {
  if (!html) return '';
  const prefix = String.raw`(<br\s*\/?>\s*|^)`;
  return html
    .replace(new RegExp(prefix + '<b>Semi-réussite</b>', 'g'), '$1' + OUTCOME_ICONS.semi_reussite + '<b>Semi-réussite</b>')
    .replace(new RegExp(prefix + '<b>Réussite</b>',      'g'), '$1' + OUTCOME_ICONS.reussite      + '<b>Réussite</b>')
